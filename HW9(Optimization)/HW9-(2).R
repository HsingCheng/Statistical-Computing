...
mini<-function(x1,x2,alpha){
    y<-((x1-alpha*gradient(x1,x2)[1,])-2)^2+((x1-alpha*gradient(x1,x2)[1,])-2*(x2-alpha*gradient(x1,x2)[2,]))^2
    return(y)
}
...
error<-10^-10
Q<-matrix(c(4,-4,-4,8),nrow = 2,byrow = TRUE)
xb<-matrix(c(0,3),nrow=2)
d<- -gradient(xb[1,],xb[2,])
conv<-FALSE
while(!conv){
  dk<-d
  cat("now dk is:",dk,"\n")
  alpha_hat<- (t(-gradient(xb[1,],xb[2,]))%*%dk)/(t(dk)%*%Q%*%dk)
  alpha_hat<-as.numeric(alpha_hat)
  xn<-xb+alpha_hat*dk
  xa<-xn
  lk<- (t(gradient(xa[1,],xa[2,]))%*%Q%*%dk)/(t(dk)%*%Q%*%dk)
  lk<-as.numeric(lk)
  d<- -gradient(xa[1,],xa[2,])+lk*dk
  r<-sqrt(sum((xb-xa)^2))
  cat("now iteration time is:",n,"the distance of value is:",r,"the function value is:",f(xa[1,],xa[2,]),"\n")
  xb<-xa
  if(r<error| n>150) (conv<-TRUE)
